...
import { CheckCircle2, Circle, ChevronDown, ChevronUp, ExternalLink, Plus, Loader2, Lock } from 'lucide-react'
        ...
        <div className="space-y-4">
          {activeRoadmap.milestones.map((milestone, idx) => {
            ...
            return (
              ...
              >
                {/* Timeline dot */}
                <div className={`absolute left-2.5 w-5 h-5 rounded-full border-2 flex items-center justify-center hidden md:flex z-10 bg-[#0a0a1a] ${
                ...
                <div className={`milestone-card ${isExpanded ? 'border-primary-500/40' : ''}`}>
                  {/* Milestone header */}
                  <div
                  ...
                  >
                    <div className="flex items-start gap-3 flex-1">
                      <button
                        onClick={e => { e.stopPropagation(); if (canComplete) completeMutation.mutate({ roadmapId: activeRoadmap.id, milestoneId: milestone.id }) }}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {milestone.topics.slice(0, 4).map(t => (
                            <span key={t} className="text-xs px-2 py-0.5 rounded-md bg-white/5 text-gray-300">{t}</span>
                          ))}
                                ...
                                {milestone.resources.map((r, ri) => (
                                  <div key={ri} className="flex items-start justify-between bg-white/5 rounded-xl p-3 gap-3">
                                    <div className="flex-1">
                                      ...
                                      {r.why_recommended && (
                                        <p className="text-xs text-gray-400 mt-1">💡 {r.why_recommended}</p>
                                      )}
                                    </div>
                                    {r.url && (
